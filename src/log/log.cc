...
DEF_int32(min_log_level, 0,
          ">>#0 write logs at or above this level, 0-4 "
          "(debug|info|warning|error|fatal)");
DEF_int32(max_log_size, 4096, ">>#0 max size of a single log");
DEF_int64(max_log_file_size, 256 << 20,
          ">>#0 max size of log file, default: 256MB");
DEF_uint32(max_log_file_num, 8, ">>#0 max number of log files");
DEF_uint32(max_log_buffer_size, 32 << 20,
           ">>#0 max size of log buffer, default: 32MB");
...
DEF_bool(log_compress, false,
         ">>#0 if true, compress rotated log files with xz");
...
static fastring _co_log_xx = []() {
  _is_safe_to_start = true;
  return "";
}();

#ifdef _WIN32
LONG WINAPI
_co_on_exception(PEXCEPTION_POINTERS p); // handler for win32 exceptions

void _co_set_except_handler() { SetUnhandledExceptionFilter(_co_on_exception); }
#else
...
private:
  struct LevelLog {
    ...
    char time_str[27]; // "2023-02-28 16:59:11.526156"
    size_t bytes;
    size_t counter;
...
Global::Global() : check_failed(false), logger(NULL) {
#ifndef _WIN32
  if (!__sys_api(write)) {
    auto r = ::write(-1, 0, 0);
    (void)r;
...
Cleanup::~Cleanup() { global().logger->stop(); }
...
bool Logger::start() {
  ...
  if (atomic_bool_cas(&_log_thread, (void *)0, (void *)8)) {
    ...
    atomic_store(&_log_thread,
                 co::static_new<Thread>(&Logger::thread_fun, this));
  } else {
...
// if signal_safe is true, try to call only async-signal-safe api in this
...
void Logger::thread_fun() {
  ...
  while (!_stop) {
    ...
    tscns::tscns().calibrate(); // calibrate tscns
    global().log_time->update();
...
void Logger::write_tlogs(co::array<PerTopic *> &v, LogTime *t) {
  for (size_t i = 0; i < v.size(); ++i) {
    ...
    if (FLG_cout)
      fwrite(pt->logs.data(), 1, pt->logs.size(), stderr);
...
void Logger::push_fatal_log(char *s, size_t n) {
  ...
  if (!FLG_cout)
    fwrite(s, 1, n, stderr);
  if (global().log_file->open(NULL, fatal, t))
    global().log_file->write(s, n, t);
...
inline uint32 LogFile::day_in_path(const fastring &path) {
  ...
  if (path.size() > n)
    god::byte_cp<4>(&x, path.data() + path.size() - n);
  return x;
}
...
bool LogFile::check_config(const char *topic, int level, LogTime *t) {
  ...
  if (!_path_base.empty() && _path_base.back() != '/')
    _path_base << '/';
  if (topic) {
  ...
  if (level != xx::fatal) {
    ...
    if (f) {
      ...
      for (auto &x : v)
        _old_paths.push_back(std::move(x));
    }
  }
...
fs::file &LogFile::open(const char *topic, int level, LogTime *t) {
  ...
  if (!topic) {
    if (!fs::exists(FLG_log_dir))
      fs::mkdir((char *)FLG_log_dir.c_str(), true);
  } else {
    s << FLG_log_dir;
    if (!s.empty() && s.back() != '/')
      s << '/';
  ...
  if (level != xx::fatal) {
    ...
    if (_file.open(_path.c_str(), 'a') && new_file) {
      char x[27] = {0}; // "2023-02-28 15:15:59.064373"
...
void LogFile::write(const char *p, size_t n, LogTime *t) {
  ...
  if (_file || this->open(NULL, 0, t)) {
    ...
    if (size >= (uint64)FLG_max_log_file_size)
      _file.close();
  }
}
...
FailureHandler::FailureHandler() : _stack_trace(log::stack_trace()) {
...
void FailureHandler::on_failure(int sig) {
  ...
  if (_stack_trace)
    _stack_trace->dump_stack(&f, g.check_failed ? 7 : (sig == SIGABRT ? 4 : 3));
  if (f) {
    f.write('\n');
    f.close();
  }
...
int FailureHandler::on_exception(PEXCEPTION_POINTERS p) {
  ...
  if (err[0] == 'U')
    s << (void *)(size_t)p->ExceptionRecord->ExceptionCode;
  s << '\n';
  if (f)
    f.write(s);
  log2stderr(s.data(), s.size());
...
void on_signal(int sig) { global().failure_handler->on_signal(sig); }

void on_failure(int sig) { global().failure_handler->on_failure(sig); }
...
LevelLogSaver::LevelLogSaver(const char *file, int len, unsigned int line,
    ...
    : _s(log_stream()) {
  _n = _s.size();
  _s.resize(_n + 27); // make room for: "I2023-02-28 15:15:59.064373"
  _s[_n] = "DIWE"[level];
  fastring time = tscns::str();
  memcpy((char *)(_s.data() + 1), time.data(), time.size()); // "2023-02-28 15:36:59.439451" log time accurate to
                       // microseconds
...
TLogSaver::TLogSaver(const char *file, int len, unsigned int line,const char *topic)
  ...
  _n = _s.size();
  _s.resize(_n + (LogTime::t_len)); // make room for: "2023-02-28 15:15:59.064373"
  fastring time = tscns::str();
...
void exit() { xx::global().logger->stop(); }
