...
/* Perform a critical factorization of NEEDLE, of length NEEDLE_LEN.
   ...
   *PERIOD to the global period of the right half.
   ...
   periodicity.  */
static size_t FN_NAME(critical_factorization)(
    const unsigned char *needle, size_t needle_len, size_t *period
) {
...
static RETURN_TYPE FN_NAME(two_way_short_needle)(
    ...
    if (CMP_FUNC (needle, needle + period, suffix) == 0) {
        ...
        while (AVAILABLE (haystack, haystack_len, j, needle_len)) {
            ...
            i = MAX (suffix, memory);
            while (
                i < needle_len && (CANON_ELEMENT (needle[i]) == CANON_ELEMENT (haystack[i + j]))
            ) ++i;
            if (needle_len <= i) {
                /* Scan for matches in left half.  */
                i = suffix - 1;
                while (
                    memory < i + 1 && (CANON_ELEMENT (needle[i]) == CANON_ELEMENT (haystack[i + j]))
                ) --i;
        ...
        while (AVAILABLE (haystack, haystack_len, j, needle_len)) {
            ...
            if (needle_len <= i) {
                ...
                i = suffix - 1;
                while (
                    i != SIZE_MAX && (CANON_ELEMENT (needle[i]) == CANON_ELEMENT (haystack[i + j]))
                ) --i;
...
static RETURN_TYPE FN_NAME(two_way_long_needle)(
    const unsigned char *haystack, size_t haystack_len,
		const unsigned char *needle, size_t needle_len
) {
    size_t i; /* Index into current byte of NEEDLE.  */
    ...
    if (CMP_FUNC (needle, needle + period, suffix) == 0) {
        ...
        while (AVAILABLE (haystack, haystack_len, j, needle_len)) {
            ...
            i = MAX (suffix, memory);
            while (
                i < needle_len - 1 && (CANON_ELEMENT (needle[i]) == CANON_ELEMENT (haystack[i + j]))
            ) ++i;
            if (needle_len - 1 <= i) {
                /* Scan for matches in left half.  */
                i = suffix - 1;
