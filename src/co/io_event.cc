...
io_event::io_event(sock_t fd, _ev_t ev)
    ...
    _info = (PerIoInfo*) co::alloc_aligned(sizeof(PerIoInfo), co::cache_line_size);
...
io_event::io_event(sock_t fd, int n)
    ...
    _info = (PerIoInfo*) co::alloc_aligned(sizeof(PerIoInfo) + n, co::cache_line_size);
...
io_event::io_event(sock_t fd, _ev_t ev, const void* buf, int size, int n)
    ...
    if (!sched->on_stack(buf)) {
        ...
        _info = (PerIoInfo*) co::alloc_aligned(sizeof(PerIoInfo) + n + size, co::cache_line_size);
        memset(_info, 0, sizeof(PerIoInfo) + n);
