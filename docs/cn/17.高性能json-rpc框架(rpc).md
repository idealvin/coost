...
- rpc_max_log_size
  ...
  指定 rpc log 的最大大小，默认为 1k。rpc log 超过此长度时，截取前面的一部分。

- rpc_max_json_buffer_size

  指定 `json::parse` 分配 buffer 的最大大小，默认为 0，buffer 大小为 json 字符串的长度。这块 buffer 用于保存 json object 中所有的 key，详细原理见 [json 16.5](16.高效json库(json).md/#165-字符串转-json)。

  有些用户可能会通过 rpc 传输 `base64` 加密的图片等数据，导致 json object 非常大，但它内部 key 的长度并不大，在这种情况下，建议将此配置项设为一个较小的值，以提升 rpc 性能。

- rpc_retry

  启用 rpc 重试功能，适用于 rpc client，默认为 false。若启用，当 rpc 请求因连接断开失败时，重新发送一次 rpc 请求。
