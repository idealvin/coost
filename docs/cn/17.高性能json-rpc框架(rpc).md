...
- rpc_max_json_parse_buffer_size
  ...
  指定 `json::parse` 分配 buffer 的最大大小，默认为 4k。这个值必须足够大，以容纳 json object 中所有的 key，详细原理见 [json 16.5](16.高效json库(json).md/#165-字符串转-json)。

- rpc_max_log_size

  指定 rpc log 的最大大小，默认为 1k。rpc log 太长时，截取前面的一部分。
