...
- The coroutine thread is called a coroutine dispatch thread.
- There can be multiple coroutine scheduling threads in a process, each scheduling thread is independent of each other.
- There can be multiple coroutines in a dispatch thread, but only one coroutine runs at the same time.
- When a coroutine hangs, the scheduling thread automatically switches to other coroutines in that thread.
- Coroutine switching is performed in user mode, faster than switching between threads.

Coroutines are very suitable for writing network programs. They are one of the best network programming methods. Coroutines write code in a synchronous manner without nested asynchronous callbacks, which greatly reduces the programmer's thinking burden.
...
- Coroutines in the same thread share a stack. When the coroutine is suspended, the data on the stack will be saved to the coroutine, and the data will be restored to the stack when it is switched back. This method greatly reduces the memory footprint and a single machine Easily create millions of coroutines.
- New coroutines can be created anywhere (including in coroutines).
...
The relevant code for context switching in the co coroutine library is taken from [tbox](https://github.com/tboox/tbox/), while tbox refers to [boost context](https://www.boost.org/doc/libs/1_70_0/libs/context/doc/html/index.html), thank you!
...
Actual tests found that creating objects of type `std::function` is expensive, so go() is especially for types `void f()`,` void f(void *)`, `void T::f() ` The functions are optimized. In practical applications, these three types of functions should be used preferentially.

Strictly speaking, the go() method just allocates a `callback` to a dispatch thread. The actual creation of the coroutine is done by the dispatch thread. However, from the user's point of view, it can be logically considered that go() creates a coroutine and assigns it to a designated scheduling thread, waiting to be executed.
...
The user cannot directly control the coroutine, but can control when the coroutine function ends execution. The co library only provides users with two non-go coroutine API:
...
- When `sleep` is called in a coroutine, the dispatch thread will suspend the current coroutine and switch to other coroutine execution.
- `stop` does not stop the coroutine, but exits all scheduling threads. Server programs are generally not terminal, and this method is rarely used.
...
When the API wrapped by co is blocked by io, the scheduling thread will automatically suspend the current coroutine and switch to other coroutine execution.

#### 15.4.1 Commonly used socket APIs

co provides more than a dozen commonly used socket APIs:
...
- `tcp_socket`, `udp_socket` are used to create sockets. The created sockets are non-blocking on Linux/Mac platforms and [overlapped](https://support.microsoft.com/en-us/help/181611/socket-overlapped-io-versus-blocking-nonblocking-mode).
...
- `shutdown` specifies the closing direction with a single character `@c`, `'r'` turns off reading,`'w'` turns off writing, and reading and writing is turned off by default.
...
Calling the socket API of the co library in the coroutine will not block, but the native socket API is called in the third party library and may still be blocked. In order to solve this problem, hook-related APIs are required, forcing the third-party library to call the APIs after the hook.
...
Threads support the `TLS` mechanism, and coroutines can also support similar `CLS` mechanisms, but considering that millions of coroutines may be created in the system, CLS does not seem to be very efficient. The co library eventually abandoned CLS and implemented `Pool instead` Class:
...
Pool mainly provides two methods: `pop` and `push`. The former pulls an element from the Pool and the latter returns the element to the Pool.

The Pool class is coroutine-safe. The methods in the calling class do not need to be locked, but must be used in the coroutine environment. There are multiple pools in the internal implementation of Pool, and different threads use different pools.
...
p.push(rds);                       // Run out of redis and put it back into the pool
...
After the elements in the pool are used up, they need to be pushed back manually, which may cause some troubles. To this end, the co library provides the `Kakalot` class (Kakalot), which pulls elements from Pool when constructing, and puts elements back to Pool when destructing, and also comes with pointer properties, you can use pointer Use Kakalot the same way.
...
void f() {
    co::Kakalot<Redis> rds(p);         // Kakarot is now a Redis* pointer
    if (rds == NULL) rds = new Redis;  // Reassign Kakarot
    rds->get("xx");                    // Kakarot is really strong
}

go(f);
```

Using CLS mechanism, 100w coroutine needs to establish 100w connection, but using pool mechanism, 100w coroutine may only need to share a small number of connections. Pool seems more efficient and reasonable than CLS, which is why this coroutine library does not support CLS.
...
Coroutine stack size, default is 1M. Each dispatch thread allocates a stack, and coroutines within the dispatch thread share this stack.

- tcp_max_recv_size

`co::recv` The maximum data length that can be received at a time. The default is 1M. If it exceeds this size, it will be received in batches.

- tcp_max_send_size

`co::send` The maximum data length that can be sent at a time. The default is 1M. If it exceeds this size, it will be sent in batches.
