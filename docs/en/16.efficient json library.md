...
- Features of the json library
    ...
    - Json has a built-in reference count. The copy operation only increments the reference count (**non-atomic operation, not thread-safe**), no memory copy is performed.
...
```cpp
Json x = json::array();      // create empty array, different from null
...
To improve parsing performance, `json::parse()` allocates only one large enough memory to save all keys, instead of allocating memory for each key separately. The size of the allocated memory is default the length of the incoming string. It may cause a waste of memory when the string is long. To this end, a new versions of `json::parse()` was provided:

```cpp
Value parse(const char* s, size_t n, size_t keys_len);
```

The third parameter `keys_len` specifies the memory size for the keys. This value is at least the sum of the lengths of all keys in the json object (including `'\0'` at the end of each key), plus another 4 bytes to save the reference counts. Below is an example:

```cpp
fastring s = "{\"hello\": 23, \"world\": {\"xxx\": 99}}";

// keys_len is not specified, allocate s.size() bytes for the keys by default.
Json x = json::parse(s.data(), s.size());

// Allocate 20 bytes for the keys.
// At least 20 bytes is required, 16 bytes for "hello", "world", "xxx", and 4 bytes for the reference count.
