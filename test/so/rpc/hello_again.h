...
class HelloAgainClient {
  public:
    HelloAgainClient(const char* ip, int port, bool use_ssl=false)
        : _rpc_cli(ip, port, use_ssl), _serv_name("xx.HelloAgain") {
    }

    HelloAgainClient(const HelloAgainClient& c)
