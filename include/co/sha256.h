...
inline co::string sha256digest(const void* s, size_t n) {
    co::string x;
    x.resize(32);
    sha256digest(s, n, x.data());
    return x;
}

inline co::string sha256digest(const char* s) {
    return sha256digest(s, strlen(s));
}

inline co::string sha256digest(const co::string& s) {
    return sha256digest(s.data(), s.size());
}

inline co::string sha256digest(const std::string& s) {
    return sha256digest(s.data(), s.size());
}
...
inline co::string sha256sum(const void* s, size_t n) {
    co::string x;
    x.resize(64);
    sha256sum(s, n, x.data());
    return x;
}

inline co::string sha256sum(const char* s) {
    return sha256sum(s, strlen(s));
}

inline co::string sha256sum(const co::string& s) {
    return sha256sum(s.data(), s.size());
}

inline co::string sha256sum(const std::string& s) {
    return sha256sum(s.data(), s.size());
}
