...
class JAlloc {
  public:
    ...
    void* alloc_jblock(size_t cap=15) {
        void* p;
        if (!_jb.empty()) {
            p = _jb.back();
            _jb.pop_back();
...
class Root {
  ...
  private:
    ...
    uint32 _make_array(uint32 cap=0) {
        const uint32 index = _alloc_header();
        if (cap == 0) {
            (new (_jb.at(index)) _Header(kArray))->index = 0;
        } else {
            const uint32 q = _alloc_queue(cap);
            (new (_jb.at(index)) _Header(kArray))->index = q;
        }
        return index;
    }
    ...
    uint32 _make_object(uint32 cap=0) {
        const uint32 index = _alloc_header();
        if (cap == 0) {
            (new (_jb.at(index)) _Header(kObject))->index = 0;
        } else {
            const uint32 q = _alloc_queue(cap * 2);
            (new (_jb.at(index)) _Header(kObject))->index = q;
        }
        return index;
    }
...
inline void Value::_Json2str(fastream& fs, bool debug) const {
    return _root->_Json2str(fs, debug, _index);
}

inline void Value::_Json2pretty(fastream& fs, int indent, int n) const {
    return _root->_Json2pretty(fs, indent, n, _index);
}

inline void Value::str(fastream& fs) const {
    return this->_Json2str(fs, false);
}
...
//typedef jsonv2::Root Json;

inline fastream& operator<<(fastream& fs, const jsonv2::Root& x) {
    x.dbg(fs);
    return fs;
}

inline fastream& operator<<(fastream& fs, const jsonv2::Value& x) {
    x.dbg(fs);
    return fs;
}
